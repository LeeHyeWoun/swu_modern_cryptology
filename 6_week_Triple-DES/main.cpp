...
/* 함수 선언 */
void Triple_DES_Enc( BYTE* p_text, BYTE* result, BYTE* key1, BYTE* key2 );		// 3-DES 암호화 함수
void Triple_DES_Dec( BYTE* c_text, BYTE* result, BYTE* key1, BYTE* key2 );		// 3-DES 복호화 함수
void DES_Enc( BYTE* p_text, BYTE* result, BYTE* key );					// DES 암호화 함수
void DES_Dec( BYTE* c_text, BYTE* result, BYTE* key );					// DES 복호화 함수
...
UINT cir_shift28( UINT n, int r );										// 28 bit 순환 시프트 함수
...
int main()
{
	...
	Triple_DES_Enc( p_text, c_text, key1, key2 );	// DES 암호화
	...
	Triple_DES_Dec( c_text, d_text, key1, key2 );	// DES 복호화
